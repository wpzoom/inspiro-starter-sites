...
class WpzoomImporter {
	...
	protected function __construct() {
		...
		add_action( 'wp_ajax_iss_import_demo_data', array( $this, 'import_demo_data_ajax_callback' ) );
		add_action( 'wp_ajax_iss_import_customizer_data', array( $this, 'import_customizer_data_ajax_callback' ) );
		add_action( 'wp_ajax_iss_after_import_data', array( $this, 'after_all_import_data_ajax_callback' ) );
		...
		add_action( 'wp_ajax_iss_delete_imported_demo', array( $this, 'delete_imported_demo_ajax_callback' ) );

		add_action( 'wp_import_insert_post', [ $this, 'save_wp_navigation_import_mapping' ], 10, 4 );
		add_action( 'iss/after_import', [ $this, 'fix_imported_wp_navigation' ] );
	...
	public function create_plugin_page() {
		...
		$this->plugin_page = add_submenu_page(
			...
			Helpers::apply_filters( 'iss/plugin_page_display_callback_function', array( $this, 'display_plugin_page' ) )
		);
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		if ( $this->plugin_page === $hook || 'inspiro_page_inspiro-demo' == $hook ) {
			...
			wp_enqueue_script( 
				'iss-importer-js', 
				INSPIRO_STARTER_SITES_URL . 'components/importer/assets/js/wpzoom-importer.js', 
				array( 'jquery' ), 
			...
			wp_localize_script( 'iss-importer-js', 'iss',
				array(
					'ajax_url'         => admin_url( 'admin-ajax.php' ),
					'ajax_nonce'       => wp_create_nonce( 'iss-ajax-verification' ),
					'import_files'     => $this->import_files,
					'wp_customize_on'  => Helpers::apply_filters( 'iss/enable_wp_customize_save_hooks', false ),
			...
			wp_enqueue_style( 
				'iss-importer-css', 
				INSPIRO_STARTER_SITES_URL . 'components/importer/assets/css/wpzoom-importer.css', 
				array(), 
				INSPIRO_STARTER_SITES_VERSION 
			);
		}
	...
	public function import_demo_data_ajax_callback() {
		...
		//ini_set( 'memory_limit', Helpers::apply_filters( 'iss/import_memory_limit', '350M' ) );
		...
		if ( ! $use_existing_importer_data ) {
			...
			if ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: iss/import_files.
		...
		Helpers::set_iss_import_data_transient( $this->get_current_importer_data() );

		if ( ! $this->before_import_executed ) {
			$this->before_import_executed = true;

			/**
			 * 2). Execute the actions hooked to the 'iss/before_content_import_execution' action:
			 *
			 * Default actions:
			 * 1 - Before content import WP action (with priority 10).
			 */
			Helpers::do_action( 'iss/before_content_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		}
		...
		/**
		 * 4). Execute the actions hooked to the 'iss/after_content_import_execution' action:
		 *
		 * Default actions:
		 ...
		 */
		Helpers::do_action( 'iss/after_content_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		...
		if ( false !== Helpers::has_action( 'iss/after_all_import_execution' ) ) {
	...
	public function import_customizer_data_ajax_callback() {
		...
		if ( $this->use_existing_importer_data() ) {
			 ...
			 */
			Helpers::do_action( 'iss/customizer_import_execution', $this->selected_import_files );
		}
	...
	public function after_all_import_data_ajax_callback() {
		...
		if ( $this->use_existing_importer_data() ) {
			 ...
			 */
			Helpers::do_action( 'iss/after_all_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		}
	...
	private function final_response() {
		...
		delete_transient( 'iss_importer_data' );
		delete_transient( 'iss_importer_data_failed_attachment_imports' );
		delete_transient( 'iss_import_menu_mapping' );
		delete_transient( 'iss_import_posts_with_nav_block' );
		...
		$response['message'] = '<img class="iss-imported-content-imported iss-imported-content-imported--success" src="' . esc_url( INSPIRO_STARTER_SITES_URL . 'components/importer/assets/images/success.svg' ) . '" alt="' . esc_attr__( 'Successful Import', 'inspiro-starter-sites' ) . '">';
	...
	private function use_existing_importer_data() {
		if ( $data = get_transient( 'iss_importer_data' ) ) {
	...
	public function setup_plugin_with_filter_data() {
		...
		$this->import_files = Helpers::validate_import_file_info( Helpers::apply_filters( 'iss/import_files', array() ) );

		/**
		 * Register all default actions (before content import, widget, customizer import and other actions)
		 * to the 'before_content_import_execution' and the 'iss/after_content_import_execution' action hook.
		 */
		$import_actions = new ImportActions();
		$import_actions->register_hooks();

		// Importer options array.
		$importer_options = Helpers::apply_filters( 'iss/importer_options', array(
			'fetch_attachments' => true,
		) );

		// Logger options for the logger used in the importer.
		$logger_options = Helpers::apply_filters( 'iss/logger_options', array(
	...
	public function start_notice_output_capturing() {
		...
		echo '<div class="iss-notices-wrapper js-iss-notice-wrapper">';
	}
	...
	public function save_wp_navigation_import_mapping( $post_id, $original_id, $postdata, $data ) {
		...
		if ( $postdata['post_type'] !== 'wp_navigation' ) {
			...
			if ( strpos( $postdata['post_content'], '<!-- wp:navigation' ) !== false ) {
				...
				$iss_post_nav_block = get_transient( 'iss_import_posts_with_nav_block' );

				if ( empty( $iss_post_nav_block ) ) {
					$iss_post_nav_block = [];
				}

				$iss_post_nav_block[] = $post_id;

				set_transient( 'iss_import_posts_with_nav_block', $iss_post_nav_block, HOUR_IN_SECONDS );
			}
		} else {
			 ...
			 * in transient.
			 */
			$iss_menu_mapping = get_transient( 'iss_import_menu_mapping' );

			if ( empty( $iss_menu_mapping ) ) {
				$iss_menu_mapping = [];
			}
			...
			set_transient( 'iss_import_menu_mapping', $iss_menu_mapping, HOUR_IN_SECONDS );
		}
	}
	...
	public function fix_imported_wp_navigation() {
		...
		$nav_import_mapping = get_transient( 'iss_import_menu_mapping' );

		// Get the post IDs that needs to be updated.
		$posts_nav_block = get_transient( 'iss_import_posts_with_nav_block' );
