<?php
/**
 * Static functions used in the WPZI plugin.
 *
 * @package wpzi
 */
...
class Helpers {
	...
	public static function download_import_files( $import_file_info ) {
		...
		$import_file_info = self::apply_filters('wpzi/pre_download_import_files', $import_file_info);
		...
		if ( empty( $import_file_info['import_file_url'] ) ) {
			...
			$content_filename = self::apply_filters( 'wpzi/downloaded_content_file_prefix', 'demo-content-import-file_' ) . self::$demo_import_start_time . self::apply_filters( 'wpzi/downloaded_content_file_suffix_and_file_extension', '.xml' );
		...
		if ( ! empty( $import_file_info['import_widget_file_url'] ) ) {
			...
			$widget_filename = self::apply_filters( 'wpzi/downloaded_widgets_file_prefix', 'demo-widgets-import-file_' ) . self::$demo_import_start_time . self::apply_filters( 'wpzi/downloaded_widgets_file_suffix_and_file_extension', '.json' );
		...
		if ( ! empty( $import_file_info['import_customizer_file_url'] ) ) {
			...
			$customizer_filename = self::apply_filters( 'wpzi/downloaded_customizer_file_prefix', 'demo-customizer-import-file_' ) . self::$demo_import_start_time . self::apply_filters( 'wpzi/downloaded_customizer_file_suffix_and_file_extension', '.dat' );
		...
		if ( ! empty( $import_file_info['import_redux'] ) && is_array( $import_file_info['import_redux'] ) ) {
			...
			foreach ( $import_file_info['import_redux'] as $index => $redux_item ) {
				$redux_filename = self::apply_filters( 'wpzi/downloaded_redux_file_prefix', 'demo-redux-import-file_' ) . $index . '-' . self::$demo_import_start_time . self::apply_filters( 'wpzi/downloaded_redux_file_suffix_and_file_extension', '.json' );
		...
		if ( ! empty( $import_file_info['import_wpforms_file_url'] ) ) {
			...
			$wpforms_filename = self::apply_filters( 'wpzi/downloaded_wpforms_file_prefix', 'demo-wpforms-import-file_' ) . self::$demo_import_start_time . self::apply_filters( 'wpzi/downloaded_wpforms_file_suffix_and_file_extension', '.json' );
	...
	public static function get_log_path() {
		...
		$upload_path = self::apply_filters( 'wpzi/upload_file_path', trailingslashit( $upload_dir['path'] ) );

		$log_path = $upload_path . self::apply_filters( 'wpzi/log_file_prefix', 'log_file_' ) . self::$demo_import_start_time . self::apply_filters( 'wpzi/log_file_suffix_and_file_extension', '.txt' );
	...
	public static function register_file_as_media_attachment( $log_path ) {
		...
		$filetype  = wp_check_filetype( basename( $log_path ), self::apply_filters( 'wpzi/file_mimes', $log_mimes ) );
	...
	public static function get_log_url( $log_path ) {
		$upload_dir = wp_upload_dir();
		$upload_url = self::apply_filters( 'wpzi/upload_file_url', trailingslashit( $upload_dir['url'] ) );
	 ...
	 */
	public static function set_wpzi_import_data_transient( $data ) {
		set_transient( 'wpzi_importer_data', $data, 0.1 * HOUR_IN_SECONDS );
	}


	/**
	 * Backwards compatible apply_filters helper.
	 * With 3.0 we changed the filter prefix from 'pt-wpzi/' to just 'wpzi/',
	 ...
	 * With 3.0 we changed the action prefix from 'pt-wpzi/' to just 'wpzi/',
	 * but we needed to make sure backwards compatibility is in place.
	...
	public static function get_failed_attachment_imports() {
		...
		return get_transient( 'wpzi_importer_data_failed_attachment_imports' );
	}
