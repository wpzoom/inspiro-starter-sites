<?php
/**
 * Class for declaring the content importer used in the WPZOOM Demo Import plugin
 *
 * @package wpzi
 */
...
class Importer {
	...
	/**
	 * The instance of the WPZI\Logger class.
	 *
	 * @var object
	 */
	public $logger;
	...
	/**
	 * The instance of the WPZOOM Demo Import class.
	 *
	 * @var object
	 */
	private $wpzi;
	...
	public function import_content( $import_file_path ) {
		...
		if ( ! Helpers::apply_filters( 'wpzi/regenerate_thumbnails_in_content_import', true ) ) {
	...
	public function new_ajax_request_maybe( $data ) {
		...
		if ( $time > Helpers::apply_filters( 'wpzi/time_for_one_ajax_call', 25 ) ) {
			$response = array(
				'status'  => 'newAJAX',
			...
			if ( ! empty( $message ) ) {
				$this->wpzi->append_to_frontend_error_messages( $message );
			}
	...
	private function set_current_importer_data() {
		$data = array_merge( $this->wpzi->get_current_importer_data(), $this->get_importer_data() );

		Helpers::set_wpzi_import_data_transient( $data );
	}
}
