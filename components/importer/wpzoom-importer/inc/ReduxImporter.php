...
class ReduxImporter {
	...
	public static function import( $import_data ) {
		$wpzi          = OneClickDemoImport::get_instance();
		$log_file_path = $wpzi->get_log_file_path();

		// Redux plugin is not active!
		if ( ! class_exists( 'ReduxFramework' ) ) {
			$error_message = esc_html__( 'The Redux plugin is not activated, so the Redux import was skipped!', 'inspiro-toolkit' );
			...
			Helpers::append_to_file(
				$error_message,
				$log_file_path,
				esc_html__( 'Importing Redux settings' , 'inspiro-toolkit' )
			);
		...
		foreach ( $import_data as $redux_item ) {
			...
			if ( isset( $redux_framework->args['opt_name'] ) ) {
				...
				$log_added = Helpers::append_to_file( /* translators: %s - the name of the Redux option. */
					sprintf( esc_html__( 'Redux settings import for: %s finished successfully!', 'inspiro-toolkit' ), $redux_item['option_name'] ),
					$log_file_path,
					esc_html__( 'Importing Redux settings' , 'inspiro-toolkit' )
				);
			}
			else { /* translators: %s - the name of the Redux option. */
				$error_message = sprintf( esc_html__( 'The Redux option name: %s, was not found in this WP site, so it was not imported!', 'inspiro-toolkit' ), $redux_item['option_name'] );
