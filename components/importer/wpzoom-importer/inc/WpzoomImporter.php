<?php
/**
 * Main WPZOOM Demo Import plugin class/file.
 *
 * @package wpzi
 */
...
class WpzoomImporter {
	...
	public function create_plugin_page() {
		...
		$this->plugin_page = add_submenu_page(
			...
			Helpers::apply_filters( 'wpzi/plugin_page_display_callback_function', array( $this, 'display_plugin_page' ) )
		);
		...
		if ( $this->plugin_page === $hook || 'inspiro_page_inspiro-demo' == $hook || ( 'admin.php' === $hook && $this->plugin_page_setup['menu_slug'] === esc_attr( $_GET['import'] ) ) ) {
			...
			wp_localize_script( 'wpzi-importer-js', 'wpzi',
				array(
					...
					'wp_customize_on'  => Helpers::apply_filters( 'wpzi/enable_wp_customize_save_hooks', false ),
	...
	public function import_demo_data_ajax_callback() {
		...
		if ( ! $this->before_import_executed ) {
			...
			/**
			 * 2). Execute the actions hooked to the 'wpzi/before_content_import_execution' action:
			 *
			 * Default actions:
			 * 1 - Before content import WP action (with priority 10).
			 */
			Helpers::do_action( 'wpzi/before_content_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		}
		...
		/**
		 * 4). Execute the actions hooked to the 'wpzi/after_content_import_execution' action:
		 *
		 * Default actions:
		 ...
		 */
		Helpers::do_action( 'wpzi/after_content_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		...
		if ( false !== Helpers::has_action( 'wpzi/after_all_import_execution' ) ) {
	...
	public function import_customizer_data_ajax_callback() {
		...
		if ( $this->use_existing_importer_data() ) {
			 ...
			 */
			Helpers::do_action( 'wpzi/customizer_import_execution', $this->selected_import_files );
		}
	...
	public function after_all_import_data_ajax_callback() {
		...
		if ( $this->use_existing_importer_data() ) {
			 ...
			 */
			Helpers::do_action( 'wpzi/after_all_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		}
	...
	private function final_response() {
		...
		delete_transient( 'wpzi_importer_data' );
		delete_transient( 'wpzi_importer_data_failed_attachment_imports' );
		delete_transient( 'wpzi_import_menu_mapping' );
	...
	private function use_existing_importer_data() {
		if ( $data = get_transient( 'wpzi_importer_data' ) ) {
	...
	public function setup_plugin_with_filter_data() {
		...
		$this->import_files = Helpers::validate_import_file_info( Helpers::apply_filters( 'wpzi/import_files', array() ) );

		/**
		 * Register all default actions (before content import, widget, customizer import and other actions)
		 * to the 'before_content_import_execution' and the 'wpzi/after_content_import_execution' action hook.
		 */
		$import_actions = new ImportActions();
		$import_actions->register_hooks();

		// Importer options array.
		$importer_options = Helpers::apply_filters( 'wpzi/importer_options', array(
			'fetch_attachments' => true,
		) );

		// Logger options for the logger used in the importer.
		$logger_options = Helpers::apply_filters( 'wpzi/logger_options', array(
	...
	public function start_notice_output_capturing() {
		...
		echo '<div class="wpzi-notices-wrapper js-wpzi-notice-wrapper">';
	}
	...
	public function save_wp_navigation_import_mapping( $post_id, $original_id, $postdata, $data ) {
		...
		if ( $postdata['post_type'] !== 'wp_navigation' ) {
			...
			if ( strpos( $postdata['post_content'], '<!-- wp:navigation' ) !== false ) {
				...
				$wpzi_post_nav_block = get_transient( 'wpzi_import_posts_with_nav_block' );

				if ( empty( $wpzi_post_nav_block ) ) {
					$wpzi_post_nav_block = [];
				}

				$wpzi_post_nav_block[] = $post_id;

				set_transient( 'wpzi_import_posts_with_nav_block', $wpzi_post_nav_block, HOUR_IN_SECONDS );
			}
		} else {
			 ...
			 * in transient.
			 */
			$wpzi_menu_mapping = get_transient( 'wpzi_import_menu_mapping' );

			if ( empty( $wpzi_menu_mapping ) ) {
				$wpzi_menu_mapping = [];
			}
			...
			set_transient( 'wpzi_import_menu_mapping', $wpzi_menu_mapping, HOUR_IN_SECONDS );
		}
	}
	...
	public function fix_imported_wp_navigation() {
		...
		$nav_import_mapping = get_transient( 'wpzi_import_menu_mapping' );

		// Get the post IDs that needs to be updated.
		$posts_nav_block = get_transient( 'wpzi_import_posts_with_nav_block' );
