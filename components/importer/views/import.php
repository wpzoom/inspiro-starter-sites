...
<div class="wpzi wpzi--install-plugins wpz-onboard_wrapper">
	...
	<div class="wpz-onboard_header">
		...
		<div class="wpz-onboard_title-wrapper">
			...
			<h2 class="wpz-onboard_framework-version">v <?php echo esc_html( $theme->get( 'Version' ) ); ?></h2>
		</div>
		<ul class="wpz-onboard_tabs">
			<li class="wpz-onboard_tab wpz-onboard_tab-demos">
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=' . INSPIRO_TOOLKIT_SETTINGS_PAGE ) ) ?>#demo-importer" title="Demo to Import">
			...
			<li class="wpz-onboard_tab wpz-onboard_tab-premium-demos">
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=' . INSPIRO_TOOLKIT_SETTINGS_PAGE ) ) ?>#premium-demos" title="Premium Demos">
				...
				<?php esc_html_e( 'Premium Demos', 'inspiro-toolkit' ); ?>
				</a>
			</li>
		</ul>
	</div>
	...
	<div class="wpzi__content-container">
		...
		<div class="wpzi__content-container-content">
			<div class="wpzi__content-container-content--main">
				<?php if ( isset( $_GET['import'] ) ) : ?>
					<div class="wpzi-install-plugins-content js-wpzi-install-plugins-content">
						<div class="wpzi-install-plugins-content-header">
							<h2><?php esc_html_e( 'Before We Import Your Demo', 'inspiro-toolkit' ); ?></h2>
							<p>
								<?php esc_html_e( 'To ensure the best experience, installing the following plugins is strongly recommended, and in some cases required.', 'inspiro-toolkit' ); ?>
							</p>
						...
						<div class="wpzi-install-plugins-content-content">
							<?php if ( empty( $theme_plugins ) ) : ?>
								<div class="wpzi-content-notice">
									<p>
										<?php esc_html_e( 'All required/recommended plugins are already installed. You can import your demo content.' , 'inspiro-toolkit' ); ?>
									</p>
								</div>
							<?php else : ?>
								<div>
								<?php foreach ( $theme_plugins as $plugin ) : ?>
									...
									<label class="plugin-item plugin-item-<?php echo esc_attr( $plugin['slug'] ); ?><?php echo $is_plugin_active ? ' plugin-item--active' : ''; ?><?php echo ! empty( $plugin['required'] ) ? ' plugin-item--required' : ''; ?>" for="wpzi-<?php echo esc_attr( $plugin['slug'] ); ?>-plugin">
										...
										<span class="plugin-item-checkbox">
											<input type="checkbox" id="wpzi-<?php echo esc_attr( $plugin['slug'] ); ?>-plugin" name="<?php echo esc_attr( $plugin['slug'] ); ?>" <?php checked( ! empty( $plugin['preselected'] ) || ! empty( $plugin['required'] ) || $is_plugin_active ); ?><?php disabled( $is_plugin_active ); ?>>
											<span class="checkbox">
												...
												<?php if ( ! empty( $plugin['required'] ) ) : ?>
													<img src="<?php echo esc_url( WPZI_URL . 'assets/images/icons/lock.svg' ); ?>" class="wpzi-lock-icon" alt="<?php esc_attr_e( 'Lock icon', 'inspiro-toolkit' ); ?>">
												<?php endif; ?>
				...
				<?php else : ?>
					<div class="js-wpzi-auto-start-manual-import"></div>
				<?php endif; ?>

				<div class="wpzi-importing js-wpzi-importing">
					<div class="wpzi-importing-header">
						<h2><?php esc_html_e( 'Importing Content' , 'inspiro-toolkit' ); ?></h2>
						<p><?php esc_html_e( 'Please sit tight while we import your content. Do not refresh the page or hit the back button.' , 'inspiro-toolkit' ); ?></p>
					</div>
					<div class="wpzi-importing-content">
						<img class="wpzi-importing-content-importing" src="<?php echo esc_url( WPZI_URL . 'assets/images/importing.svg' ); ?>" alt="<?php esc_attr_e( 'Importing animation', 'inspiro-toolkit' ); ?>">
					</div>
				</div>

				<div class="wpzi-imported js-wpzi-imported">
					<div class="wpzi-imported-header">
						<h2 class="js-wpzi-ajax-response-title"><?php esc_html_e( 'Import Complete!' , 'inspiro-toolkit' ); ?></h2>
	...
	<?php require_once INSPIRO_TOOLKIT_PATH . 'components/admin/parts/footer.php'; ?>
</div>
